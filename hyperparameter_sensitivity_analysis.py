...
NOMINAL_PARAMS = {"learning_rate": 3e-4, "gamma": 0.999, "gae_lambda": 0.99, "clip_range": 0.2}
...
def run_clip_range_sweep():
    ...
    for clip_range in CLIP_RANGES:
        ...
        results.append((clip_range, mean, std, mean_clip_fraction))
...
def plot_sensitivity(learning_rate_results, gamma_results, clip_results, network_results):
    ...
    axes[0].errorbar(lr_values, lr_means, yerr=lr_stds, marker="o", capsize=4, label=r"Mean ± 1$\sigma$")
    ...
    axes[1].errorbar(discount_rates, gamma_means, yerr=gamma_stds, marker="o", capsize=4)
    ...
    fig.legend(reversed(handles), reversed(labels), loc="lower center", ncol=2, bbox_to_anchor=(0.5, 0.01))
...
def plot_learning_rate_histories():
    ...
    for lr in learning_rates:
        ...
        ax.plot(steps, mean_returns, marker="o", label=label)
