...
def evaluate_fixed_policy(model, env_params, n_episodes=N_EVAL_EPISODES):
    """
...
def run_robustness_sweep(model, parameter_name, values):
    results = []
    ...
    for value in values:
        ...
        metrics = evaluate_fixed_policy(model, env_params)
...
def plot_robustness(downrange_results, acceleration_results, descent_results):
    """
    Top row:
    ...
    experiments = [
        (downrange_results, "Initial downrange velocity [m/s]", NOMINAL_DOWNRANGE_VELOCITY, "Downrange velocity"),
        (acceleration_results, r"Maximum acceleration [m/s$^2$]", NOMINAL_MAX_ACCELERATION, "Control authority"),
        (descent_results, "Descent time [s]", NOMINAL_DESCENT_TIME, "Descent time"),
    ]

    for col, (results, xlabel, nominal_value, title) in enumerate(experiments):
        values = np.array([r["value"] for r in results])
        ...
        ax_safety.errorbar(values, mean_safety, yerr=std_safety, marker="o", capsize=4, label=r"Mean ± 1$\sigma$")
        ax_safety.axvline(nominal_value, linestyle="--", color="k", label="Training condition")
        ax_safety.plot(values, min_safety, marker="o", color="tab:red", linestyle="--", label="Minimum safety")
        ...
        ax_speed.errorbar(values, mean_speed, yerr=std_speed, marker="o", capsize=4, label="Mean ± std")
        ax_speed.plot(
            values, max_speed, marker="o", color="tab:orange", linestyle="--", label="Maximum touchdown speed"
        )
        ax_speed.axvline(nominal_value, linestyle="--", color="k", label="Training condition")
...
if __name__ == "__main__":
    ...
    downrange_results = run_robustness_sweep(model, "nominal_downrange_velocity", DOWNRANGE_VELOCITIES)
    save_results("robustness_downrange_velocity.npz", downrange_results)
    acceleration_results = run_robustness_sweep(model, "max_acceleration", MAX_ACCELERATIONS)
    save_results("robustness_max_acceleration.npz", acceleration_results)
    descent_results = run_robustness_sweep(model, "descent_time", DESCENT_TIMES)
    save_results("robustness_descent_time.npz", descent_results)

    fig, axes = plot_robustness(downrange_results, acceleration_results, descent_results)
