...
class LunarHazardEnvironment(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.hazard_map = generate_hazard_map(size=self.map_size, random_seed=int(self.np_random.integers(0, np.iinfo(np.int32).max)))[0]
    ...
    def _get_observation(self):
        ...
        local_patch = get_local_patch(
            self.hazard_map, scaled_position[0], scaled_position[1], self.patch_size
        )
        ...
        observation = np.concatenate(
            [scaled_position, scaled_velocity, [scaled_time], local_patch.flatten()]
        )
    ...
    def _get_terminal_reward(self) -> float:
        ...
        if not np.all((-1.0 <= normalized_position) & (normalized_position <= 1.0)):  # outside the map
            safety = 0.0
        else:
            ix = int(np.round((normalized_position[0] + 1) / 2 * (self.map_size - 1)))
            iy = int(np.round((normalized_position[1] + 1) / 2 * (self.map_size - 1)))
            safety = float(self.hazard_map[iy, ix])
        ...
        return self.safety_weight * safety - self.velocity_weight * velocity_penalty - self.target_weight * target_penalty
